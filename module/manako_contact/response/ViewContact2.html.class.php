...
class ViewContact2 extends HtmlResponse {

   function TemplateModule() {
      $this->SetTemplateBasedir(Configuration::Instance()->GetValue( 'application', 'docroot').
         'module/manako_contact/template');
      $this->SetTemplateFile('view_contact2.html');
   }
   ...
   function ParseTemplate($data = NULL) {
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('manako_contact', 'contact2', 'view', 'html') );
      ...
      if (empty($data['dataContact'])) {
         ...
         for($i=1;$i<count($listContact)+1;$i++){
            ...
            */
            ...
            @$urlAccept = 'manako_contact|deleteContact|do|';
            @$urlReturn = 'manako_contact|contact|view|';
            $label = 'Contact';
            $dataName = $listContact[$i]['contactNameFirst'] .' ' .$listContact[$i]['contactNameLast'];
            $listContact[$i]['url_delete'] = Dispatcher::Instance()->GetUrl('confirm', 'confirmDelete', 'do', 'html').'&urlDelete='. $urlAccept.'&urlReturn='.$urlReturn.'&id='.$idEnc.'&label='.$label.'&dataName='.$dataName;
