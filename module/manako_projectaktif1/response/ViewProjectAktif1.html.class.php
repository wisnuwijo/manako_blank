...
require_once Configuration::Instance()->GetValue( 'application', 'docroot') . 'module/manako_projectaktif1/business/'.Configuration::Instance()->GetValue( 'application','db_conn',0,'db_type').'/ProjectAktif1.class.php';

class ViewProjectAktif1 extends HtmlResponse {

   function TemplateModule() {
      $this->SetTemplateBasedir(Configuration::Instance()->GetValue( 'application', 'docroot').
         'module/manako_projectaktif1/template');
      $this->SetTemplateFile('view_projectaktif1.html');
   }
   ...
   function ParseTemplate($data = NULL) {
      $this->mrTemplate->AddVar('content', 'URL_SEARCH', Dispatcher::Instance()->GetUrl('manako_projectaktif1', 'projectAktif1', 'view', 'html') );
      //$this->mrTemplate->AddVar('content', 'PROJECT_URL_ADD', Dispatcher::Instance()->GetUrl('manako_projectaktif1', 'inputProject', 'view', 'html') );
      ...
      if (empty($data['dataProject'])) {
         ...
         for ($i=0; $i<$len; $i++) {
            ...
            //Reformat
            $listProject[$no]['projectDateConEnd']=date('d-m-Y', strtotime($dataProject[$no]['projectDateConEnd']));
            if (strtotime($listProject[$no]['projectDateMtcEnd'])<=0) {
              $listProject[$no]['projectDateMtcEnd'] ='-';
            } else {
              $listProject[$no]['projectDateMtcEnd']=date('d-m-Y', strtotime($listProject[$no]['projectDateMtcEnd']));
            }
            if (strtotime($listProject[$no]['projectDateBast'])<=0) {
              $listProject[$no]['projectDateBast'] ='-';
            } else {
              $listProject[$no]['projectDateBast']=date('d-m-Y', strtotime($listProject[$no]['projectDateBast']));
            }
         ...
         for($i=0;$i<count($listProject);$i++){
            ...
            //$listProject[$i]['url_edit'] = Dispatcher::Instance()->GetUrl('manako_projectaktif1', 'inputProject', 'view', 'html') . '&idd=' . $idEnc;
            $listProject[$i]['url_detail'] = Dispatcher::Instance()->GetUrl('manako_projectaktif1', 'detailProjectAktif1', 'view', 'html') . '&idd=' . $idEnc;
            /*
            @$urlAccept = 'manako_projectaktif1|deleteProject|do|';
            @$urlReturn = 'manako_projectaktif1|project|view|';
            $label = 'Project';
