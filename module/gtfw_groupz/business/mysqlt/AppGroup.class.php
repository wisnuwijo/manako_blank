...
class AppGroup extends Database {
   ...
   protected $mSqlFile= 'module/gtfw_groupz/business/mysqlt/appgroup.sql.php';
...
//===GET===  
   ...
   function GetComboUnitKerja($applicationId){
	...
	function GetAllAksiByGroupId($groupId){
   	return $this->Open($this->mSqlQueries['get_all_aksi_by_group_id'],array($groupId));
   }
...
//===DO===
   ...
   function DoAddGroupModuleFromGtfwMenu($dmMenuId, $groupId='',$setting) {
   	if($setting == '1'){
   		$param_sql2 = "AND m.AksiId IS NULL";
   		$param_sql1 = "AND a.AksiId IS NULL";
   	}else{
   		$param_sql1 = "";
   		$param_sql2 = "";
   	}
      if ($groupId!= '') {
      	$query_name1 = "do_add_group_module_from_gtfw_menu";
      	$this->mSqlQueries[$query_name1] = str_replace('[SQL_WHERE_1]', $param_sql1, $this->mSqlQueries[$query_name1]);
         $result = $this->ExecuteInsertQuery($this->mSqlQueries[$query_name1], array($groupId, $dmMenuId));
      } else {
      	$query_name2 = "do_add_group_module_from_gtfw_menu_new_group";
      	$this->mSqlQueries[$query_name2] = str_replace('[SQL_WHERE_2]', $param_sql2, $this->mSqlQueries[$query_name2]);
         $result = $this->ExecuteInsertQuery($this->mSqlQueries[$query_name2], array($dmMenuId));
      }
      
      return $result;
   }
   
   function DoAddGroupModule($groupId, $applicationId, $newGroup= false) {
      if ($newGroup === false) {
         $result = $this->Execute($this->mSqlQueries['do_add_group_module'], array($groupId, $applicationId));
      } else {
         $result = $this->ExecuteInsertQuery($this->mSqlQueries['do_add_group_module_newgroup'], array($applicationId));
      }
      return $result;
   }
   
	function DoAddGroupModule2($groupId, $moduleId, $newGroup= false) {
      if ($newGroup === false) {
         $result = $this->Execute($this->mSqlQueries['do_add_group_module2'], array($groupId, $moduleId));
      } else {
         $result = $this->ExecuteInsertQuery($this->mSqlQueries['do_add_group_module_newgroup2'], array($moduleId));
      }
      return $result;
   }
