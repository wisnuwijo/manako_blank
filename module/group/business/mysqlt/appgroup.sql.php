...
FROM
   gtfw_menu m 
   LEFT JOIN gtfw_group_menu ggm 
      ON ggm.MenuMenuId = m.MenuId 
      AND GroupId = '%s' 
WHERE m.IsShow = 'Yes' 
   AND m.ApplicationId = '%s' 
ORDER BY MenuParentId,
   m.MenuName 
";
$sql['get_all_privilege_old'] = 
   "SELECT 
  	  m.MenuId AS menu_id,
  	  m.MenuName AS menu_name,
  	  m.MenuParentId AS menu_parent_id,
...
$sql['get_all_aksi_by_group_id_old']="
SELECT a.MenuId,a.ModuleId, a.LabelAksi, b.AksiLabel, (SELECT COUNT(c.ModuleId) FROM gtfw_group_module c WHERE c.ModuleId = a.ModuleId AND c.GroupId = '%s') AS Checked
FROM gtfw_module a
LEFT JOIN gtfw_aksi b ON b.AksiId = a.AksiId
WHERE MenuId IS NOT NULL
ORDER BY a.AksiId ASC
";
...
$sql['do_add_group_module_from_gtfw_menu_new_group'] = "
   ...
   WHERE m.MenuId='%s' /*NEW*/ AND m.AksiId IS NULL
   GROUP BY m.ModuleId
";
...
$sql['do_add_group_module_from_gtfw_menu'] = "
   ...
   SELECT
      '%s', a.ModuleId
   FROM gtfw_module a
   WHERE a.MenuId='%s' /*NEW*/ AND a.AksiId IS NULL
";
